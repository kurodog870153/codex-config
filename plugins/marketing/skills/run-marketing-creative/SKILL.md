...
6. In this coordinated workflow, defer all snapshot writes until both the copy and images are approved; the specialized skills must return handoffs without saving their own snapshots.
...
5. Use `$develop-marketing-copy` in deferred-save mode to establish the brief, explore directions, draft, revise, obtain explicit copy approval, and return an approved handoff without writing a snapshot.
6. Preserve the approved copy handoff exactly.
7. Use `$create-marketing-image` in deferred-save mode to discuss visual directions, approve a generation specification, generate variants, review them, obtain explicit image approval, and return approved prompts and images without writing a snapshot.
8. If either specialized skill attempts to save an intermediate snapshot, stop before the write and restate that the coordinated workflow owns the single final snapshot.
9. If either specialized skill is unavailable, stop and identify the missing skill rather than recreating its workflow.
...
5. For a new campaign with no existing snapshot, write the approved copy, prompts, and images together as the complete `v01`; do not create a copy-only intermediate version.
6. Include `manifest.yaml`, `creative-brief.md`, `copy/`, `image-prompts/`, and `images/`.
7. Name copy files `copy/<channel>.md`.
8. Name prompt files `image-prompts/<channel>-<format>.md`.
9. Name image variants `images/<channel>-<format>-NN.<original-extension>` with zero-padded variant numbers.
10. Preserve the image generator's original file format and prefer PNG when a format choice is available.
11. Show the exact files, copy-forward operations, and risks, then obtain explicit confirmation before writing.
