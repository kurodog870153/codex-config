...
1. Spring Data JPA／Jakarta Persistence／Hibernate 實作必須遵循下列規則：
    ...
    4. 嚴格依 TASK 實作 access type、Entity／table、主鍵與產生策略、欄位、null、長度、precision／scale、唯一性、預設值、複合主鍵、識別碼不可變性、`equals`／`hashCode`、enum、converter、日期時間與 schema 映射；不得依 JPA、Hibernate 或資料庫預設自行補足未固定契約。
    ...
    15. Auditing、Entity listener、邏輯刪除、租戶及資料權限須依 TASK 固定的目前使用者來源、查詢條件與 bulk／native 行為執行；Entity 不得直接讀取 Controller、HTTP Session 或安全框架上下文。
