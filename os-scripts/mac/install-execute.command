...
source_skill="$project_directory/skills/execute"
source_work="$project_directory/work/execute"
...
if (( ${#optional_rules[@]} > 0 )); then
    printf 'Optional execute work rules:\n'
    ...
    printf 'No optional execute work rules were found. General will be installed.\n'
fi
...
target_skill="$target_directory/skills/execute"
target_work="$target_directory/work/execute"

if [[ -e "$target_skill" || -e "$target_work" ]]; then
    read -r -p 'Execute skill or work rules already exist. Merge and overwrite matching files? [y/N] ' overwrite_execute
    if [[ ! "$overwrite_execute" =~ ^[Yy]$ ]]; then
        printf 'Operation cancelled.\n'
        read -r -p 'Press Enter to close...' _
        exit 0
    fi
...
if ! mkdir -p -- "$target_skill"; then
    printf 'Error: failed to create the execute skill directory.\n'
    read -r -p 'Press Enter to close...' _
    exit 1
fi

if ! cp -R -- "$source_skill/." "$target_skill/"; then
    printf 'Error: failed to copy the execute skill.\n'
    read -r -p 'Press Enter to close...' _
    exit 1
fi

if ! mkdir -p -- "$target_work/general" || ! cp -f -- "$source_general" "$target_work/general/AGENTS.md"; then
    printf 'Error: failed to copy the general execute work rule.\n'
    read -r -p 'Press Enter to close...' _
    exit 1
fi
...
for rule_path in "${selected_rules[@]}"; do
    if ! mkdir -p -- "$target_work/$rule_path" || ! cp -f -- "$source_work/$rule_path/AGENTS.md" "$target_work/$rule_path/AGENTS.md"; then
        printf 'Error: failed to copy the execute work rule "%s".\n' "$rule_path"
        read -r -p 'Press Enter to close...' _
        exit 1
    fi
done

printf 'The execute skill and general work rule were installed in "%s".\n' "$target_directory"
