...
source_skill="$project_directory/skills/task"
source_work="$project_directory/work/task"
...
if (( ${#optional_rules[@]} > 0 )); then
    printf 'Optional task work rules:\n'
    ...
    printf 'No optional task work rules were found. General will be installed.\n'
fi
...
target_skill="$target_directory/skills/task"
target_work="$target_directory/work/task"

if [[ -e "$target_skill" || -e "$target_work" ]]; then
    read -r -p 'Task skill or work rules already exist. Merge and overwrite matching files? [y/N] ' overwrite_task
    if [[ ! "$overwrite_task" =~ ^[Yy]$ ]]; then
        printf 'Operation cancelled.\n'
        read -r -p 'Press Enter to close...' _
        exit 0
    fi
...
if ! mkdir -p -- "$target_skill"; then
    printf 'Error: failed to create the task skill directory.\n'
    read -r -p 'Press Enter to close...' _
    exit 1
fi

if ! cp -R -- "$source_skill/." "$target_skill/"; then
    printf 'Error: failed to copy the task skill.\n'
    read -r -p 'Press Enter to close...' _
    exit 1
fi

if ! mkdir -p -- "$target_work/general" || ! cp -f -- "$source_general" "$target_work/general/AGENTS.md"; then
    printf 'Error: failed to copy the general task work rule.\n'
    read -r -p 'Press Enter to close...' _
    exit 1
fi
...
for rule_path in "${selected_rules[@]}"; do
    if ! mkdir -p -- "$target_work/$rule_path" || ! cp -f -- "$source_work/$rule_path/AGENTS.md" "$target_work/$rule_path/AGENTS.md"; then
        printf 'Error: failed to copy the task work rule "%s".\n' "$rule_path"
        read -r -p 'Press Enter to close...' _
        exit 1
    fi
done

printf 'The task skill and general work rule were installed in "%s".\n' "$target_directory"
