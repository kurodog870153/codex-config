...
set "source_skill=%project_directory%\skills\plan"
set "source_work=%project_directory%\work\plan"
...
echo Optional plan work rules:
for /l %%I in (1,1,!option_count!) do echo   %%I. !option_%%I!
goto prompt_selection

:no_optional_rules
echo No optional plan work rules were found. General will be installed.
...
:check_existing
if exist "!target_directory!\skills\plan\" goto confirm_overwrite
if exist "!target_directory!\work\plan\" goto confirm_overwrite
goto copy_files

:confirm_overwrite
choice /c YN /n /m "Plan skill or work rules already exist. Merge and overwrite matching files? [Y/N] "
if errorlevel 2 goto cancelled

:copy_files
if not exist "!target_directory!\skills\plan\" (
    mkdir "!target_directory!\skills\plan"
    if errorlevel 1 goto failed_skills_directory
)
xcopy "%source_skill%\*" "!target_directory!\skills\plan\" /E /I /Y /Q >nul
if errorlevel 1 goto failed_skill_copy

if not exist "!target_directory!\work\plan\general\" mkdir "!target_directory!\work\plan\general"
if errorlevel 1 goto failed_work_directory
copy /Y "%source_general%" "!target_directory!\work\plan\general\AGENTS.md" >nul
if errorlevel 1 goto failed_general_copy

for /l %%I in (1,1,!copy_rule_count!) do (
    set "rule_path=!copy_rule_%%I!"
    if not exist "!target_directory!\work\plan\!rule_path!\" mkdir "!target_directory!\work\plan\!rule_path!"
    if errorlevel 1 goto failed_work_directory
    copy /Y "%source_work%\!rule_path!\AGENTS.md" "!target_directory!\work\plan\!rule_path!\AGENTS.md" >nul
    if errorlevel 1 goto failed_rule_copy
)

echo The plan skill and general work rule were installed in "!target_directory!".
...
:failed_skill_copy
echo Error: failed to copy the plan skill.
pause
exit /b 1

:failed_work_directory
echo Error: failed to create a plan work rule directory.
pause
exit /b 1

:failed_general_copy
echo Error: failed to copy the general plan work rule.
pause
exit /b 1

:failed_rule_copy
echo Error: failed to copy the plan work rule "!rule_path!".
pause
exit /b 1
